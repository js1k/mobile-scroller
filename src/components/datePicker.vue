    ...
    <mt-popup v-model="visible" :closeOnClickModal="closeOnClickModal" position="bottom" class="date-picker"
        ...
        <mt-picker
            ...
            <div class="buttons">
                ...
                <span v-show="customDateError2" class="custom-error">最多选择365天</span>
                <!-- <span v-show="customDateError3" class="custom-error">最多选择{{stepDay}}天</span> -->
            </div>
            <p v-if="dateType !== 'custom'" class="normal">{{currentValue | dateFormat(dateType)}}</p>
            <p v-else class="custom">
        <span
...
export default {
    ...
    methods: {
        ...
        confirm () {
            ...
            if (this.dateType === 'custom') {
                ...
                // if (difValue > this.stepDay) {
                //     this.customDateError3 = true;
                //     return;
                // }
                if (difValue > 365) {
    ...
    created () {
        // this.selectStart = new Date(Date.now() - 16000 * 3600 * 24);
        this.selectStart = new Date(Date.now() - this.stepDate * 3600 * 24);
